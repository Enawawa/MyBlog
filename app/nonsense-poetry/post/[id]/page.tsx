...
import AIPoemLoader from "./AIPoemLoader";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  if (params.id.startsWith("ai-")) {
    return {
      title: "AI 废话诗 — 废话文学",
      description: "由 Gemini AI 生成的打工人废话文学短诗",
    };
  }
...
export default function PoemDetailPage({ params }: Props) {
  const isAI = params.id.startsWith("ai-");

  if (isAI) {
    return <AIPoemLoader id={params.id} />;
  }
  ...
  const recommendations =
    related.length >= 2
      ? related
      : allToday.filter((p) => p.id !== poem.id).slice(0, 4);
